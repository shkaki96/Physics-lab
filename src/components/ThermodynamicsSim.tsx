...
import { Play, Pause, RotateCcw, BookmarkCheck, Flame, Snowflake, Gauge, Wind, Activity } from 'lucide-react';
...
export default function ThermodynamicsSim({ lang, onLogMeasurement }: Props) {
  ...
  const drawChamber = () => {
    ...
    // Render Particles
    particlesRef.current.forEach((p) => {
      // Particle color based on speed / temperature
  ...
  const handleLog = () => {
    onLogMeasurement({
      ...
      parameters: {
        ...
        'Particle Count N': `${particleCount}`,
      ...
      <div className="lg:col-span-8 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 font-mono">
          <div className="p-3 rounded-xl bg-zinc-900/70 border border-zinc-800">
            <span className="text-[10px] text-zinc-400 block">{t.pressure}</span>
            <span className="text-sm font-bold text-amber-400">{calculatedPressure.toFixed(2)} kPa</span>
          </div>

          <div className="p-3 rounded-xl bg-zinc-900/70 border border-zinc-800">
            <span className="text-[10px] text-zinc-400 block">{t.volume}</span>
            <span className="text-sm font-bold text-sky-400">{volume.toFixed(1)} L</span>
          </div>

          <div className="p-3 rounded-xl bg-zinc-900/70 border border-zinc-800">
            <span className="text-[10px] text-zinc-400 block">{t.temperature}</span>
            <span className="text-sm font-bold text-rose-400">{temperature} K</span>
          </div>

          <div className="p-3 rounded-xl bg-zinc-900/70 border border-zinc-800">
            <span className="text-[10px] text-zinc-400 block">{t.particles}</span>
        ...
        <div className="p-4 rounded-2xl bg-zinc-900/80 border border-zinc-800 space-y-4">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs">
              <span className="text-zinc-400">{t.volume} (V)</span>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs">
              <span className="text-zinc-400">{t.temperature} (T)</span>
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs">
              <span className="text-zinc-400">{t.particles} (N)</span>
