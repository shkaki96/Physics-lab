...
import { Search, BookOpen, Calculator, Copy, Check, Sparkles, Plus, Layers } from 'lucide-react';
...
export default function FormulaSheet({ lang }: Props) {
  const t = TRANSLATIONS[lang].formulas;
  ...
  const filtered = FORMULAS.filter((f) => {
    ...
    const matchesSearch =
      ...
      (f.nameKu && f.nameKu.toLowerCase().includes(q)) ||
      f.formula.toLowerCase().includes(q) ||
      f.variables.some((v) => 
        v.nameAr.toLowerCase().includes(q) || 
        v.nameEn.toLowerCase().includes(q) || 
        (v.nameKu && v.nameKu.toLowerCase().includes(q)) ||
            ...
            <div className="space-y-3">
              ...
              <h3 className="text-base font-bold text-zinc-100">
                {lang === 'ar' 
                  ? item.nameAr 
                  : lang === 'kmr'
                  ? (item.nameKmr || item.nameKu || item.nameEn)
                  : lang === 'ku' && item.nameKu 
                  ? item.nameKu 
                  : item.nameEn}
              </h3>
              ...
              <p className="text-xs text-zinc-400 leading-relaxed">
                {lang === 'ar' 
                  ? item.descriptionAr 
                  : lang === 'kmr'
                  ? (item.descriptionKmr || item.descriptionKu || item.descriptionEn)
                  : lang === 'ku' && item.descriptionKu 
                  ? item.descriptionKu 
                  : item.descriptionEn}
              </p>
            </div>
