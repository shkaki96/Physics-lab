...
import { Play, RotateCcw, BookmarkCheck, Target, Plus, Trash2, Crosshair, Scale, CheckCircle2 } from 'lucide-react';
...
export default function CenterOfMassSim({ lang, onLogMeasurement }: Props) {
  ...
  useEffect(() => {
    ...
    if (mode === 'particles') {
      ...
      ctx.font = 'bold 11px sans-serif';
      ctx.fillText(lang === 'ar' ? 'مركز الثقل الهندسي ⨁' : 'Geometric Center of Gravity ⨁', cmPlateX + 12, cmPlateY + 4);
    } else {
      ...
      ctx.fillText(
        lang === 'ar'
          ? '✓ النظام في حالة اتزان دوراني كامل (Στ = 0) حول نقطة الارتكاز X_cm'
          : '✓ System in perfect rotational equilibrium (Στ = 0) about fulcrum at X_cm',
        originX,
        height - 30
      );
    }
      ...
      <div className="lg:col-span-2 space-y-4">
        <div className="relative rounded-2xl bg-zinc-900 border border-zinc-800 p-4 shadow-xl overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-base font-bold text-zinc-100">
                  {lang === 'ar' ? 'تحديد مركز الكتلة للأجسام الصلبة (Xcm = Σmi xi / Σmi)' : lang === 'ku' ? 'دیاریکردنی چەقی بارستایی تەنەکان (Xcm = Σmi xi / Σmi)' : 'Center of Mass Determination (Xcm = Σmi xi / Σmi)'}
                </h3>
                <p className="text-xs text-zinc-400">
                  {lang === 'ar' ? 'اسحب الكتل بالماوس على الشبكة لرؤية موضع مركز الكتلة ⨁ يتحرك في الزمن الحقيقي' : 'Drag masses on the grid to observe real-time shift of Center of Mass ⨁'}
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3">
            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'إحداثي X لمركز الكتلة' : 'X_cm Coordinate'}</div>
              <div className="text-base font-bold font-mono text-red-400">
                {xCM.toFixed(3)} <span className="text-xs text-zinc-400">m</span>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'إحداثي Y لمركز الكتلة' : 'Y_cm Coordinate'}</div>
              <div className="text-base font-bold font-mono text-red-400">
                {yCM.toFixed(3)} <span className="text-xs text-zinc-400">m</span>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'الكتلة الكلية (M_total)' : 'Total Mass'}</div>
              <div className="text-base font-bold font-mono text-sky-400">
                {totalMass.toFixed(2)} <span className="text-xs text-zinc-400">kg</span>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'العزم الأول (Σ mi xi)' : 'First Moment (Σ mi xi)'}</div>
              <div className="text-base font-bold font-mono text-amber-400">
                {sumMx.toFixed(2)} <span className="text-xs text-zinc-400">kg·m</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-red-950/20 border border-red-800/30 text-xs text-zinc-300 space-y-2">
          <div className="font-semibold text-red-300 flex items-center gap-1.5">
            ...
            <span>{lang === 'ar' ? 'القوانين الفيزيائية لمركز الكتلة والاتزان:' : 'Center of Mass & Equilibrium Physics:'}</span>
          </div>
          <p>
            {lang === 'ar'
              ? 'مركز الكتلة هو النقطة المتوسطة الموزونة بكتل الأجسام، حيث يتصرف الجسم وكأن كتلته كلها متركزة فيها وتؤثر القوى الخارجية عليها: X_cm = Σ(m_i·x_i)/Σm_i و Y_cm = Σ(m_i·y_i)/Σm_i. عند وضع نقطة الارتكاز تحته مباشرة، يكون عزم الدوران المحصل صفراً فيحدث اتزان سكوني تام.'
              : 'The Center of Mass is the mass-weighted average position of all particles: X_cm = Σ(mi xi)/Σmi and Y_cm = Σ(mi yi)/Σmi. Placing a pivot directly below the center of mass yields zero net gravitational torque, achieving static rotational equilibrium.'}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-5 shadow-xl space-y-5">
          <h4 className="text-sm font-bold text-zinc-200 pb-2 border-b border-zinc-800">
            {lang === 'ar' ? 'أسلوب العرض والتجربة' : 'Experiment Mode'}
          </h4>
          ...
          <div className="grid grid-cols-3 gap-1.5 text-xs">
            ...
            >
              {lang === 'ar' ? 'خيط الشاقول' : 'Plumb Line'}
            </button>
            <button
            ...
            >
              {lang === 'ar' ? 'ميزان الارتكاز' : 'Fulcrum'}
            </button>
          </div>

          {mode === 'plumbline' && (
            <div className="space-y-2 p-3 rounded-xl bg-zinc-950/70 border border-zinc-800">
              <label className="text-xs text-zinc-300 font-semibold">{lang === 'ar' ? 'نقطة التعليق الحرة:' : 'Suspension Pivot Point:'}</label>
              <div className="grid grid-cols-3 gap-2 text-xs">
                <button
                  onClick={() => setPivotPoint('A')}
                  className={`p-2 rounded-lg font-bold ${pivotPoint === 'A' ? 'bg-sky-600 text-white' : 'bg-zinc-800 text-zinc-300'}`}
                >
                  {lang === 'ar' ? 'الثقب A' : 'Pivot A'}
                </button>
                <button
                  onClick={() => setPivotPoint('B')}
                  className={`p-2 rounded-lg font-bold ${pivotPoint === 'B' ? 'bg-sky-600 text-white' : 'bg-zinc-800 text-zinc-300'}`}
                >
                  {lang === 'ar' ? 'الثقب B' : 'Pivot B'}
                </button>
                <button
                  onClick={() => setPivotPoint('C')}
                  className={`p-2 rounded-lg font-bold ${pivotPoint === 'C' ? 'bg-sky-600 text-white' : 'bg-zinc-800 text-zinc-300'}`}
                >
                  {lang === 'ar' ? 'الثقب C' : 'Pivot C'}
                </button>
              </div>
              <p className="text-[11px] text-zinc-400 pt-1">
                {lang === 'ar'
                  ? 'عند تعليق الصفيحة من نقطتين مختلفتين، يمر خط الشاقول الرأسي دائماً بمركز الثقل، ونقطة تقاطع الخطين تحدده عملياً بدقة.'
                  : 'Suspending from two different pivots draws vertical plumb lines that intersect exactly at the center of gravity.'}
              </p>
            </div>
          )}
          ...
          {mode === 'particles' && (
            <div className="space-y-3 pt-2 border-t border-zinc-800">
              <div className="flex items-center justify-between">
                <span className="text-xs text-zinc-300 font-bold">{lang === 'ar' ? 'الكتل النقطية:' : 'Point Masses:'}</span>
                <button
                  onClick={handleAddMass}
                  disabled={masses.length >= 6}
                  className="px-2 py-1 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-xs flex items-center gap-1 disabled:opacity-40"
                >
                  <Plus className="w-3.5 h-3.5" />
                  <span>{lang === 'ar' ? 'إضافة كتلة' : 'Add Mass'}</span>
                </button>
              </div>
          ...
          >
            ...
            <span>
              {logged
                ...
                : (lang === 'ar' ? 'تسجيل إحداثيات مركز الكتلة' : 'Log Center of Mass Data')}
            </span>
          </button>
        </div>
      </div>
