...
import { Magnet, Zap, Compass, RotateCcw, Activity } from 'lucide-react';
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">
              {lang === 'ar' ? 'المغناطيسات والكهرومغناطيسات (الملف اللولبي)' : 'Magnets and Electromagnets (Solenoid)'}
            </h3>
        ...
        <div className="lg:col-span-8 bg-slate-950 border border-slate-800 rounded-2xl p-4 flex flex-col items-center justify-center min-h-[320px] relative overflow-hidden">
          ...
          <div className="relative z-10 flex items-center justify-center gap-6 my-4">
            ...
            <div className="flex flex-col items-center bg-slate-900 border border-amber-500/40 p-3 rounded-xl shadow-lg">
              ...
              <span className="text-[10px] text-slate-400">{lang === 'ar' ? 'مصدر التيار' : 'DC Supply'}</span>
            </div>
            ...
            <div className="relative flex items-center justify-center">
              {/* Iron Rod Core */}
              <div
                className={`w-44 h-10 rounded-lg transition-all flex items-center justify-between px-3 ${
                  hasIronCore
                    ? 'bg-gradient-to-r from-slate-600 via-slate-400 to-slate-600 border-2 border-slate-300 shadow-lg'
                    : 'bg-slate-900/60 border border-dashed border-slate-700'
                }`}
              >
                ...
                <span className="text-[9px] text-slate-950/80 font-mono font-bold">
                  {hasIronCore ? (lang === 'ar' ? 'قلب حديدي' : 'Iron Core') : (lang === 'ar' ? 'فراغ / هواء' : 'Air Core')}
                </span>
          ...
          <div className="flex items-center gap-2 mt-2 bg-slate-900/90 border border-slate-800 px-3 py-1.5 rounded-xl">
            <span className="text-xs text-slate-300">
              {lang === 'ar' ? 'مشابك الورق المنجذبة:' : 'Attracted Paperclips:'}
            </span>
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-3.5 space-y-2.5">
            <h4 className="text-xs font-bold text-purple-300 uppercase tracking-wider">
              {lang === 'ar' ? 'المخرجات المحسوبة' : 'Calculated Outputs'}
            </h4>
            <div className="space-y-1.5 font-mono text-xs">
              <div className="flex justify-between p-2 rounded-lg bg-slate-900 border border-slate-800">
                <span className="text-slate-400">{lang === 'ar' ? 'مجال الملف (B_core):' : 'Coil Field (B):'}</span>
                <span className="text-purple-300 font-bold">{B_core} mT</span>
              </div>
              <div className="flex justify-between p-2 rounded-lg bg-slate-900 border border-slate-800">
                <span className="text-slate-400">{lang === 'ar' ? 'المجال عند البوصلة:' : 'Field at Compass:'}</span>
                <span className="text-cyan-300 font-bold">{B_at_distance} mT</span>
              </div>
              <div className="flex justify-between p-2 rounded-lg bg-slate-900 border border-slate-800">
                <span className="text-slate-400">{lang === 'ar' ? 'معامل النفاذية (μᵣ):' : 'Rel. Permeability:'}</span>
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-3.5 space-y-3">
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-slate-300">{lang === 'ar' ? 'التيار الكهربائي (I):' : 'Current (I):'}</span>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-slate-300">{lang === 'ar' ? 'عدد اللفات (N):' : 'Coil Turns (N):'}</span>
            ...
            <div className="flex items-center justify-between pt-1">
              <span className="text-xs text-slate-300">
                {lang === 'ar' ? 'إدخال قلب حديدي داخل الملف:' : 'Insert Iron Core:'}
              </span>
              <button
              ...
              >
                {hasIronCore ? (lang === 'ar' ? 'مفعل (نعم)' : 'Active') : (lang === 'ar' ? 'معطل (هواء)' : 'Air Core')}
              </button>
            </div>
          </div>
        </div>
