        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">
              {lang === 'ar'
                ? 'محاكاة المحول الكهربائي (Electrical Transformer)'
                : lang === 'ku'
                ? 'مۆدێلی گۆڕەری کارەبایی (Transformer)'
                : lang === 'kmr'
                ? 'Modelkirina Transformatorek Elektrîkî'
                : 'Electrical Transformer Simulation'}
            </h3>
        ...
        <div className="lg:col-span-8 bg-slate-950 border border-slate-800 rounded-2xl p-4 flex flex-col justify-between min-h-[340px] relative overflow-hidden">
          ...
          <div className="flex items-center justify-between z-10 text-xs">
            ...
            >
              {isStepUp
                ? lang === 'ar'
                  ? '⚡ محول رافع للجهد (Step-Up Transformer)'
                  : '⚡ Step-Up Transformer (Vₛ > Vₚ)'
                : isStepDown
                ? lang === 'ar'
                  ? '🔋 محول خافض للجهد (Step-Down Transformer)'
                  : '🔋 Step-Down Transformer (Vₛ < Vₚ)'
                : lang === 'ar'
                ? '⚖️ محول عازل 1:1 (Isolation Transformer)'
                : '⚖️ 1:1 Isolation Transformer'}
            </span>
            ...
            <svg viewBox="0 0 100 80" className="w-full h-full max-w-lg">
              ...
              <g>
                <text x="18" y="8" fill="#f87171" fontSize="3" fontWeight="bold" textAnchor="middle">
                  {lang === 'ar' ? 'الملف الابتدائي' : 'Primary'}
                </text>
              ...
              <g>
                <text x="82" y="8" fill="#38bdf8" fontSize="3" fontWeight="bold" textAnchor="middle">
                  {lang === 'ar' ? 'الملف الثانوي' : 'Secondary'}
                </text>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-slate-900/90 border border-slate-800 p-2.5 rounded-xl text-center text-xs font-mono">
            <div>
              <span className="text-slate-400 text-[10px] block">{lang === 'ar' ? 'الجهد الابتدائي (Vₚ)' : 'Primary Voltage'}</span>
              <span className="text-rose-400 font-bold">{primaryVoltage} V</span>
            </div>
            <div>
              <span className="text-slate-400 text-[10px] block">{lang === 'ar' ? 'الجهد الثانوي (Vₛ)' : 'Secondary Voltage'}</span>
              <span className="text-sky-400 font-bold">{secondaryVoltage.toFixed(1)} V</span>
            </div>
            <div>
              <span className="text-slate-400 text-[10px] block">{lang === 'ar' ? 'التيار الثانوي (Iₛ)' : 'Secondary Current'}</span>
              <span className="text-emerald-400 font-bold">{secondaryCurrent.toFixed(2)} A</span>
            </div>
            <div>
              <span className="text-slate-400 text-[10px] block">{lang === 'ar' ? 'القدرة الناتجة (Pₛ)' : 'Secondary Power'}</span>
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-3.5 space-y-3">
            <h4 className="text-xs font-bold text-amber-300 uppercase tracking-wider">
              {lang === 'ar' ? 'متغيرات المحول والملفات' : 'Transformer Parameters'}
            </h4>

            {/* Primary Voltage */}
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-slate-300">{lang === 'ar' ? 'جهد المصدر (Vₚ):' : 'Primary Voltage (Vₚ):'}</span>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-slate-300">{lang === 'ar' ? 'لفات الابتدائي (Nₚ):' : 'Primary Turns (Nₚ):'}</span>
                <span className="font-mono text-rose-400 font-bold">{primaryTurns} turns</span>
              </div>
              <input
                type="range"
                min="50"
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-slate-300">{lang === 'ar' ? 'لفات الثانوي (Nₛ):' : 'Secondary Turns (Nₛ):'}</span>
                <span className="font-mono text-sky-400 font-bold">{secondaryTurns} turns</span>
              </div>
              <input
                type="range"
                min="50"
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-slate-300">{lang === 'ar' ? 'مقاومة الحمل (R_L):' : 'Load Resistance (R_L):'}</span>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-slate-300">{lang === 'ar' ? 'كفاءة المحول (η):' : 'Efficiency (η):'}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-2">
        <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
          <div className="text-[11px] text-slate-400 font-medium">
            {lang === 'ar' ? 'التيار الابتدائي المسحوب (Iₚ)' : 'Primary Current (Iₚ)'}
          </div>
          <div className="text-lg font-mono font-bold text-rose-400">
            {primaryCurrent.toFixed(2)} A
          </div>
          <div className="text-[10px] text-slate-500 font-mono">Iₚ = Pₚ / Vₚ (قانون حفظ الطاقة)</div>
        </div>

        <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
          <div className="text-[11px] text-slate-400 font-medium">
            {lang === 'ar' ? 'القدرة الابتدائية المغذية (Pₚ)' : 'Primary Power (Pₚ)'}
          </div>
          <div className="text-lg font-mono font-bold text-amber-400">
            {primaryPower.toFixed(1)} W
          </div>
          <div className="text-[10px] text-slate-500 font-mono">Pₚ = Pₛ / η ({efficiency}% كفاءة)</div>
        </div>

        <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
          <div className="text-[11px] text-slate-400 font-medium">
            {lang === 'ar' ? 'نسبة التحويل (Transformation Ratio a)' : 'Turns Ratio (a)'}
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
          <div className="text-[11px] text-slate-400 font-medium">
            {lang === 'ar' ? 'أقصى تدفق مغناطيسي (Φ_max)' : 'Peak Magnetic Flux (Φ_max)'}
          </div>
