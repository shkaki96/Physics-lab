...
import { Play, Pause, RotateCcw, BookmarkCheck, CircleDot, Compass, Activity } from 'lucide-react';
...
export default function ArcLengthSim({ lang, onLogMeasurement }: Props) {
  const t = {
    title: lang === 'ar' ? 'العلاقة بين الزاوية بالراديان وطول القوس (s = r θ)' : lang === 'ku' ? 'پەیوەندی نێوان گۆشە بە ڕادیان و درێژی کەوانە (s = r θ)' : 'Arc Length vs Radian Angle (s = r θ)',
    shortDesc: lang === 'ar' ? 'التحقق الهندسي والفيزيائي من تعريف الراديان، قياس طول القوس s، السرعة الخطية والزاوية v = r ω.' : lang === 'ku' ? 'سەلماندنی پێناسەی ڕادیان، پێوانی درێژی کەوانە s، خێرایی هێڵی و گۆشەیی v = r ω.' : 'Geometric & physical verification of radian angle definition, arc length measurement s = r θ, and linear vs angular velocity.',
  };
  ...
  useEffect(() => {
    ...
    if (showUnrolled) {
      ...
      ctx.fillStyle = '#94a3b8';
      ctx.font = '11px sans-serif';
      ctx.fillText(lang === 'ar' ? 'فرد القوس في خط مستقيم' : lang === 'ku' ? 'ڕاکێشانی کەوانەکە بە شێوەی ڕاستەهێڵ' : 'Unrolled Arc Line (s)', trackStartX - 40, trackStartY + 25);
    }
      ...
      <div className="lg:col-span-2 space-y-4">
        <div className="relative rounded-2xl bg-zinc-900 border border-zinc-800 p-4 shadow-xl overflow-hidden">
          ...
          <div className="flex items-center justify-between pb-3 border-b border-zinc-800/80 mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-base font-bold text-zinc-100">
                  {lang === 'ar' ? 'التحقق من العلاقة بين الزاوية وطول القوس (s = r θ)' : lang === 'ku' ? 'پەیوەندی نێوان گۆشە و درێژی کەوانە (s = r θ)' : 'Arc Length vs Radian Angle (s = r θ)'}
                </h3>
                <p className="text-xs text-zinc-400">
                  {lang === 'ar' ? 'اسحب النقطة البرتقالية على المحيط لتغيير الزاوية ورؤية طول القوس وفردِه' : lang === 'ku' ? 'خاڵە پرتەقاڵییەکە ڕابکێشە بۆ گۆڕینی گۆشە و بینینی درێژی کەوانە' : 'Drag the orange handle along the circle to vary angle θ and arc length s'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>{isRotating ? (lang === 'ar' ? 'إيقاف' : 'Pause') : (lang === 'ar' ? 'دوران مستمر' : 'Rotate')}</span>
              </button>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3">
            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'الزاوية (θ بالراديان)' : 'Angle θ (rad)'}</div>
              <div className="text-lg font-bold font-mono text-sky-400">
                {angleRad.toFixed(3)} <span className="text-xs text-zinc-400">rad ({(angleRad / Math.PI).toFixed(2)}π)</span>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'طول القوس المقاس (s)' : 'Arc Length (s)'}</div>
              <div className="text-lg font-bold font-mono text-amber-400">
                {theoreticalArcLength.toFixed(3)} <span className="text-xs text-zinc-400">m</span>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'السرعة الخطية (v = rω)' : 'Linear Velocity (v)'}</div>
              <div className="text-lg font-bold font-mono text-emerald-400">
                {linearSpeed.toFixed(2)} <span className="text-xs text-zinc-400">m/s</span>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'مساحة القطاع الدائري (A)' : 'Sector Area (A)'}</div>
              <div className="text-lg font-bold font-mono text-purple-400">
                {sectorArea.toFixed(3)} <span className="text-xs text-zinc-400">m²</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-sky-950/20 border border-sky-800/30 text-xs text-zinc-300 space-y-2">
          <div className="font-semibold text-sky-300 flex items-center gap-1.5">
            ...
            <span>{lang === 'ar' ? 'التعريف الفيزيائي للراديان وطول القوس:' : lang === 'ku' ? 'پێناسەی فیزیایی ڕادیان و درێژی کەوانە:' : 'Physics Definition of Radian & Arc Length:'}</span>
          </div>
          <p>
            {lang === 'ar'
              ? 'الراديان هو الزاوية المركزية التي يقابلها قوس طوله يساوي نصف قطر الدائرة بالضبط (عندما s = r تكون θ = 1 rad). وبالتالي العلاقة العامة هي: s = r · θ (حيث θ بالراديان). والسرعة المماسية الخطية v ترتبط بالسرعة الزاوية ω عبر v = r · ω.'
              : lang === 'ku'
              ? 'ڕادیان بریتییە لەو گۆشە ناوەندییەی کە درێژی کەوانەکەی یەکسانە بە نیوەتیرەی بازنەکە (کاتێک s = r ئەوا θ = 1 rad). کەواتە هاوکێشەی گشتی بریتییە لە s = r · θ.'
              : 'One radian is the central angle subtended by an arc equal in length to the radius (s = r => θ = 1 rad). Therefore, the general arc length is s = r · θ. Tangential velocity is v = r · ω.'}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-5 shadow-xl space-y-5">
          <h4 className="text-sm font-bold text-zinc-200 pb-2 border-b border-zinc-800">
            {lang === 'ar' ? 'المعاملات وضوابط التجربة' : lang === 'ku' ? 'کۆنترۆڵ و پارامیتەرەکان' : 'Experiment Controls'}
          </h4>

          {/* Radius Control */}
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-zinc-400">{lang === 'ar' ? 'نصف القطر (r)' : 'Radius (r)'}</span>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-xs">
              <span className="text-zinc-400">{lang === 'ar' ? 'الزاوية بالدرجات (θ°)' : 'Angle in Degrees (θ°)'}</span>
          ...
          <div className="space-y-1.5">
            <label className="text-xs text-zinc-400">{lang === 'ar' ? 'زوايا قياسية شائعة بالراديان:' : 'Standard Radian Presets:'}</label>
          ...
          <div className="space-y-2 pt-2 border-t border-zinc-800">
            <div className="flex justify-between text-xs">
              <span className="text-zinc-400">{lang === 'ar' ? 'السرعة الزاوية (ω)' : 'Angular Velocity (ω)'}</span>
          ...
          <div className="space-y-2 pt-2 border-t border-zinc-800 text-xs">
            <label className="flex items-center gap-2 cursor-pointer text-zinc-300">
              ...
              />
              <span>{lang === 'ar' ? 'عرض فرد القوس إلى خط مستقيم للمقارنة' : 'Show unrolled linear arc comparison'}</span>
            </label>
            ...
            <label className="flex items-center gap-2 cursor-pointer text-zinc-300">
              ...
              />
              <span>{lang === 'ar' ? 'عرض متجه السرعة المماسية (v)' : 'Show tangential velocity vector (v)'}</span>
            </label>
          </div>
          ...
          <button
            ...
            className={`w-full py-3 px-4 rounded-xl font-bold text-xs flex items-center justify-center gap-2 shadow-lg transition-all ${
              logged
                ...
                : 'bg-gradient-to-r from-sky-600 to-indigo-600 hover:from-sky-500 hover:to-indigo-500 text-white shadow-indigo-900/30'
            }`}
          >
            ...
            <span>
              {logged
                ...
                : (lang === 'ar' ? 'تسجيل القياس في دفتر المختبر' : 'Log Measurement to Notebook')}
            </span>
          </button>
        </div>
      </div>
