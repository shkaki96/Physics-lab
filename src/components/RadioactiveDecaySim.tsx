...
const ISOTOPES: IsotopePreset[] = [
  { id: 'c14', nameAr: 'الكربون-14 (C-14)', nameEn: 'Carbon-14 (¹⁴C)', halfLifeSec: 10, displayHalfLife: '5,730 years', radiationType: 'β⁻' },
  { id: 'i131', nameAr: 'اليود-131 (I-131)', nameEn: 'Iodine-131 (¹³¹I)', halfLifeSec: 6, displayHalfLife: '8.02 days', radiationType: 'β⁻ + γ' },
  { id: 'rn222', nameAr: 'الرادون-222 (Rn-222)', nameEn: 'Radon-222 (²²²Rn)', halfLifeSec: 4, displayHalfLife: '3.82 days', radiationType: 'α' },
  { id: 'co60', nameAr: 'الكوبالت-60 (Co-60)', nameEn: 'Cobalt-60 (⁶⁰Co)', halfLifeSec: 8, displayHalfLife: '5.27 years', radiationType: 'β⁻ + γ' },
  { id: 'po218', nameAr: 'البولونيوم-218 (Po-218)', nameEn: 'Polonium-218 (²¹⁸Po)', halfLifeSec: 3, displayHalfLife: '3.10 minutes', radiationType: 'α' },
];
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-3">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-sm font-bold text-white">
              {lang === 'ar'
                ? 'محاكاة التحلل الإشعاعي وعمر النصف (Radioactive Decay)'
                : lang === 'ku'
                ? 'مۆدێلی تێکشکانی تیشکدەر و نیوەتەمەن (Half-life)'
                : lang === 'kmr'
                ? 'Modelkirina Hilweşîna Radyoaktîf'
                : 'Radioactive Decay & Half-life Simulation'}
            </h3>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={lang === 'ar' ? 'تقديم نصف عمر كامل (+1 T½)' : 'Step 1 Half-life (+1 T½)'}
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        <div className="lg:col-span-8 bg-slate-950 border border-slate-800 rounded-2xl p-4 flex flex-col justify-between min-h-[340px] relative overflow-hidden">
          ...
          <div className="flex items-center justify-between z-10 text-xs flex-wrap gap-2">
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1.5 text-emerald-400 font-semibold">
                ...
                {lang === 'ar' ? 'أنوية مشعة نشطة (N)' : 'Active Parent Nuclei'}: {activeCount}
              </span>
              <span className="flex items-center gap-1.5 text-slate-500 font-semibold">
                <span className="w-2.5 h-2.5 rounded-full bg-slate-600" />
                {lang === 'ar' ? 'أنوية وليدة مستقرة' : 'Decayed Daughter'}: {decayedCount}
              </span>
            </div>

            <span className="font-mono text-slate-300 bg-slate-900/80 px-2.5 py-1 rounded-md border border-slate-800">
              {lang === 'ar' ? 'دورات عمر النصف:' : 'Half-lives:'} <strong className="text-amber-400">{elapsedHalfLives.toFixed(2)} T₁/₂</strong>
            </span>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-slate-900/90 border border-slate-800 p-2.5 rounded-xl text-center text-xs font-mono">
            <div>
              <span className="text-slate-400 text-[10px] block">{lang === 'ar' ? 'الزمن المنقضي (t)' : 'Elapsed Time'}</span>
              <span className="text-slate-200 font-bold">{elapsedTime.toFixed(1)} s</span>
            </div>
            <div>
              <span className="text-slate-400 text-[10px] block">{lang === 'ar' ? 'الأنوية المتبقية (N)' : 'Active Nuclei'}</span>
              <span className="text-emerald-400 font-bold">{activeCount} / {initialCount}</span>
            </div>
            <div>
              <span className="text-slate-400 text-[10px] block">{lang === 'ar' ? 'النسبة المتبقية' : 'Remaining %'}</span>
              <span className="text-amber-400 font-bold">{((activeCount / initialCount) * 100).toFixed(1)}%</span>
            </div>
            <div>
              <span className="text-slate-400 text-[10px] block">{lang === 'ar' ? 'النشاط الإشعاعي (A)' : 'Activity (A)'}</span>
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-3.5 space-y-3">
            <h4 className="text-xs font-bold text-emerald-300 uppercase tracking-wider">
              {lang === 'ar' ? 'المتغيرات والنظائر' : 'Isotope & Controls'}
            </h4>

            {/* Isotope Preset Selector */}
            <div className="space-y-1">
              <label className="text-xs text-slate-300">{lang === 'ar' ? 'النظير المشع المستهدف:' : 'Radioactive Isotope:'}</label>
              <select
                value={isotopeIndex}
              ...
              >
                {ISOTOPES.map((iso, idx) => (
                  <option key={iso.id} value={idx}>
                    {lang === 'ar' ? iso.nameAr : iso.nameEn} (T½ = {iso.displayHalfLife})
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-slate-300">{lang === 'ar' ? 'عدد الأنوية الابتدائية (N₀):' : 'Initial Nuclei (N₀):'}</span>
            ...
            <div className="p-3 rounded-lg bg-slate-900 border border-slate-800 text-xs space-y-1 text-slate-300">
              ...
              <div className="flex justify-between font-mono">
                <span className="text-slate-400">{lang === 'ar' ? 'نوع الإشعاع المنبعث:' : 'Decay Mode:'}</span>
                <span className="text-sky-300 font-bold">{isotope.radiationType}</span>
              </div>
              <div className="flex justify-between font-mono">
                <span className="text-slate-400">{lang === 'ar' ? 'ثابت التحلل (λ):' : 'Decay Constant λ:'}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 pt-2">
        <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
          <div className="text-[11px] text-slate-400 font-medium">
            {lang === 'ar' ? 'الأنوية المتفككة (N_decayed)' : 'Decayed Nuclei'}
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
          <div className="text-[11px] text-slate-400 font-medium">
            {lang === 'ar' ? 'القيمة النظرية للأنوية المتبقية' : 'Theoretical N(t)'}
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
          <div className="text-[11px] text-slate-400 font-medium">
            {lang === 'ar' ? 'ثابت التحلل الإشعاعي (λ)' : 'Decay Constant (λ)'}
          </div>
        ...
        <div className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 space-y-1">
          <div className="text-[11px] text-slate-400 font-medium">
            {lang === 'ar' ? 'النشاط الإشعاعي اللحظي (A)' : 'Instantaneous Activity (A)'}
          </div>
          <div className="text-lg font-mono font-bold text-amber-400">
            {activityBq.toFixed(1)} Bq
          </div>
          <div className="text-[10px] text-slate-500 font-mono">A(t) = λ · N(t) (انحلال / ثانية)</div>
        </div>
      </div>
    </div>
  );
