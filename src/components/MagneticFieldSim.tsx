...
import { Play, RotateCcw, BookmarkCheck, Zap, Compass, Magnet, ArrowRight, Gauge, Layers } from 'lucide-react';
...
export default function MagneticFieldSim({ lang, onLogMeasurement }: Props) {
  ...
  useEffect(() => {
    ...
    if (mode === 'solenoid') {
      ...
      if (coreMaterial !== 'air') {
        ...
        ctx.fillText(coreMaterial === 'iron' ? 'SOFT IRON CORE (μr=200)' : 'FERRITE CORE (μr=50)', centerX, centerY + 4);
      }
  ...
  const handleLog = () => {
    ...
    onLogMeasurement({
      ...
      variableName: mode === 'force_wire' ? 'Magnetic Force (F)' : 'Magnetic Flux Density (B)',
      ...
      <div className="lg:col-span-2 space-y-4">
        <div className="relative rounded-2xl bg-zinc-900 border border-zinc-800 p-4 shadow-xl overflow-hidden">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-base font-bold text-zinc-100">
                  {lang === 'ar' ? 'استقصاء المجال المغناطيسي والقوة الكهرومغناطيسية' : lang === 'ku' ? 'لێکۆڵینەوە لە بواری موگناتیسی و هێزی کارۆموگناتیسی' : 'Magnetic Field & Electromagnetic Force Investigation'}
                </h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3">
            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'شدة التيار الكهربائي (I)' : 'Electric Current (I)'}</div>
              <div className="text-base font-bold font-mono text-amber-400">
                {currentI.toFixed(2)} <span className="text-xs text-zinc-400">A</span>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'كثافة التدفق المغناطيسي (B)' : 'Magnetic Flux (B)'}</div>
              <div className="text-base font-bold font-mono text-sky-400">
                {((mode === 'solenoid' ? solenoidB : straightWireB) * 1000).toFixed(2)} <span className="text-xs text-zinc-400">mT</span>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'المجال بوحدة غاوس (Gauss)' : 'Field in Gauss'}</div>
              <div className="text-base font-bold font-mono text-emerald-400">
                {((mode === 'solenoid' ? solenoidB : straightWireB) * 10000).toFixed(1)} <span className="text-xs text-zinc-400">G</span>
              </div>
            </div>

            <div className="p-3 rounded-xl bg-zinc-950/60 border border-zinc-800/60 text-center">
              <div className="text-[11px] text-zinc-400">{lang === 'ar' ? 'القوة المغناطيسية (F)' : 'Magnetic Force (F)'}</div>
              <div className="text-base font-bold font-mono text-rose-400">
                {magneticForce.toFixed(3)} <span className="text-xs text-zinc-400">N</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-amber-950/20 border border-amber-800/30 text-xs text-zinc-300 space-y-2">
          <div className="font-semibold text-amber-300 flex items-center gap-1.5">
            ...
            <span>{lang === 'ar' ? 'قوانين الكهرومغناطيسية وقاعدة اليد اليمنى:' : 'Electromagnetism Laws & Right Hand Rule:'}</span>
          </div>
          <p>
            {lang === 'ar'
              ? 'ينتج التيار الكهربائي مجالاً مغناطيسياً يتناسب طردياً مع شدة التيار I وكثافة اللفات n. في الملف اللولبي يكون المجال منتظماً داخلياً ويحسب من B = μ₀ μᵣ n I، ويتضاعف مئات المرات عند إدخال قلب من الحديد المطاوع (μᵣ = 200). وتتأثر الأسلاك الحاملة للتيار في المجال بقوة لورنتز F = I L B sinθ.'
              : 'Electric current generates a magnetic field proportional to current I and turn density n. Solenoid field is uniform inside (B = μ₀ μᵣ n I) and amplifies hundreds of times with a soft iron core. Current-carrying wires experience Lorentz force F = I L B sin θ.'}
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-5 shadow-xl space-y-5">
          <h4 className="text-sm font-bold text-zinc-200 pb-2 border-b border-zinc-800">
            {lang === 'ar' ? 'نظام التجربة والبارامترات' : 'System Mode & Parameters'}
          </h4>
          ...
          <div className="grid grid-cols-3 gap-1.5 text-xs">
            ...
            >
              {lang === 'ar' ? 'سلك مستقيم' : 'Straight Wire'}
            </button>
            <button
            ...
            >
              {lang === 'ar' ? 'قوة لورنتز F' : 'Lorentz Force'}
            </button>
          </div>
          ...
          <div className="space-y-1">
            <div className="flex justify-between text-xs">
              <span className="text-zinc-400">{lang === 'ar' ? 'شدة التيار الكهربائي (I)' : 'Electric Current (I)'}</span>
          ...
          {mode === 'solenoid' && (
            <>
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="text-zinc-400">{lang === 'ar' ? 'عدد اللفات (N)' : 'Number of Turns (N)'}</span>
              ...
              <div className="space-y-1.5">
                <label className="text-xs text-zinc-400">{lang === 'ar' ? 'مادة القلب الداخلي للملف (μᵣ):' : 'Core Material (μᵣ):'}</label>
                <div className="grid grid-cols-3 gap-1.5 text-xs">
                  <button
                    onClick={() => setCoreMaterial('air')}
                    className={`p-2 rounded-xl text-center font-mono ${coreMaterial === 'air' ? 'bg-sky-600 text-white' : 'bg-zinc-950 text-zinc-400 border border-zinc-800'}`}
                  >
                    <div>{lang === 'ar' ? 'هواء' : 'Air'}</div>
                    <div className="text-[10px]">μᵣ = 1</div>
                  </button>
                  <button
                    onClick={() => setCoreMaterial('ferrite')}
                    className={`p-2 rounded-xl text-center font-mono ${coreMaterial === 'ferrite' ? 'bg-sky-600 text-white' : 'bg-zinc-950 text-zinc-400 border border-zinc-800'}`}
                  >
                    <div>{lang === 'ar' ? 'فيريت' : 'Ferrite'}</div>
                    <div className="text-[10px]">μᵣ = 50</div>
                  </button>
                  <button
                    onClick={() => setCoreMaterial('iron')}
                    className={`p-2 rounded-xl text-center font-mono ${coreMaterial === 'iron' ? 'bg-sky-600 text-white' : 'bg-zinc-950 text-zinc-400 border border-zinc-800'}`}
                  >
                    <div>{lang === 'ar' ? 'حديد مطاوع' : 'Soft Iron'}</div>
          ...
          {mode === 'force_wire' && (
            <div className="space-y-2 pt-2 border-t border-zinc-800">
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="text-zinc-400">{lang === 'ar' ? 'المجال المغناطيسي الخارجي (B)' : 'External B Field'}</span>
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-xs">
                  <span className="text-zinc-400">{lang === 'ar' ? 'زاوية السلك مع المجال (θ)' : 'Wire Angle (θ)'}</span>
          ...
          >
            ...
            <span>
              {logged
                ...
                : (lang === 'ar' ? 'تسجيل التدفق المغناطيسي والقوة' : 'Log Magnetic Field Data')}
            </span>
          </button>
        </div>
      </div>
